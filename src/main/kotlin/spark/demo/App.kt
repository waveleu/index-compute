...
fun streamingdemo() {
    ...
    val lines = ssc.socketTextStream("127.0.0.1", 9999, StorageLevels.MEMORY_AND_DISK_SER)
...
//    Dataset统计指标：mean均值，variance方差，stddev标准差，skewness偏度，kurtosis峰度
    val result = spark.sql("select merchant_id, mean(amount) as avg_amount,variance(amount) as variance_amount,stddev(amount) as stddev_amount,skewness(amount) as skewness_amount,kurtosis(amount) as kurtosis_amount from orders where group by merchant_id")
    //val result = spark.sql("select merchant_id, date(finished_at) as fa, sum(amount) as amount, count(*) as count from orders where amount < 10000 group by merchant_id, fa having fa = '2019-05-22' limit 10 ")

    val resultDF = result.filter { value: Row ->  run {
